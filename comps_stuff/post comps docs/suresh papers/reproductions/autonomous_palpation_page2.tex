...
%%
%% Reconstruction of page 2 as study notes.
%% Prose: extracted verbatim from the PDF text layer via pdftotext.
%% Layout: single-column study-notes style, not IEEE two-column.
%% No equations, no figures on this page. Bulleted contributions
%% list; section II (Related Work) with subsections A and B;
%% section III (Methodology) begins at page end.
%% =================================================================
...
\noindent
{\footnotesize\scshape 2 \hfill IEEE Robotics and Automation Letters.\ Preprint Version.\ Accepted February, 2026}

\vspace{1.2em}

% ---------- Continuation of Introduction from page 1 ----------
\noindent
...
construct stiffness maps, where the robot explores the environment in
proportion to the Expected Information Density (EID). To plan
...
detection and boundary delineation compared to BO. To summarise, the
...
\item A convergence-based stopping criterion derived from the ergodic
metric.
\end{itemize}
...
\noindent\textit{A.~Bayesian Optimisation for Palpation}

\vspace{0.2em}

\noindent
Several studies have employed BO to guide robotic palpation to
reduce the number of probing actions while maximising information
gain. For example, Garg et al.\ [12] investigated how different
acquisition functions affect the estimated stiffness map, showing how
their choice influences both peak detection and regional estimation.
Yan et al.\ [15] highlighted a key limitation of BO: while it excels
at locating stiffness peaks, it struggles to delineate their
boundaries accurately. To address this, they proposed a two-step
approach combining BO-based search and boundary refinement using a
radial strategy originating from the centroid of the estimated
region. However, this strategy requires a reliable estimation of the
...
stiffness mapping during surgery [22]. A critical limitation of BO is
that exploration and exploitation are handled sequentially through
...
\noindent\textit{B.~Ergodic Exploration}
...
prior information was available. Conversely, our results indicate the
opposite trend, highlighting the potential of ergodic control for
robotic palpation. The main reason is that SMC often prioritises
