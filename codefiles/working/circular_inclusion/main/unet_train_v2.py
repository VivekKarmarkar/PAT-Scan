...
smoothing_sigma = 0.05  # Gaussian smoothing bandwidth for boundary noise reduction
...
def smooth_mask_differentiable(element_mask_values, centroids, sigma=0.05):
    ...
    This smooths boundary noise while staying in the computational graph.
    
    Args:
        element_mask_values: [n_elements] material mask values
        centroids: [n_elements, 2] element centroid coordinates
        sigma: smoothing bandwidth (smaller = less smoothing)
    
    Returns:
        smoothed: [n_elements] smoothed mask values
    """
    ...
    # Gaussian kernel weights: elements close together have high weights
    weights = torch.exp(-dist_matrix**2 / (2 * sigma**2))
    
    # Normalize weights so each row sums to 1
    weights = weights / weights.sum(dim=1, keepdim=True)
    
    # Smooth: each element becomes weighted average of its neighbors
...
def forward_pass_and_loss(model, coords_input, points, elements, centroids,
    ...
    # 3.5. Smooth mask to reduce boundary noise (differentiable)
...
# Main title
fig.suptitle(f'Final Results After {num_iterations} Iterations (√L2 Loss + TV Reg + Smoothing)', 
             fontsize=16, fontweight='bold', y=0.98)

plt.tight_layout()
final_viz_path = 'final_training_result_v2.png'
