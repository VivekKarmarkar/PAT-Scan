"""
U-Net Training Script for Inverse Problem - v9 WITH LEARNING RATE GRID SEARCH
...
GRID SEARCH: Optimizes hyperparameters INCLUDING LEARNING RATE before main training
"""
...
grid_search_space = {
    'learning_rate': [1e-4, 5e-4, 1e-3],  # NEW: Added learning rate to grid search!
...
num_iterations = 200
learning_rate = 1e-3  # Default, will be overwritten by grid search if enabled
log_every = 10
...
print("="*70)
print("U-NET INVERSE PROBLEM TRAINING - V9 WITH LEARNING RATE GRID SEARCH")
...
if GRID_SEARCH_ENABLED:
    ...
    for config_idx, config in enumerate(all_combinations):
        ...
        print(f"  learning_rate={config_dict['learning_rate']:.0e}, lambda_tv={config_dict['lambda_tv']}")
        ...
        # Use the learning rate from the grid search configuration
    ...
    print(f"  learning_rate: {best_config['learning_rate']:.0e}")
    ...
    with open(grid_search_save_path, 'w') as f:
        ...
        for idx, result in enumerate(sorted_results):
            ...
            f.write(f"  learning_rate: {result['config']['learning_rate']:.0e}\n")
    ...
    # Use best hyperparameters for main training (INCLUDING LEARNING RATE)
...
# Use the optimal learning rate from grid search
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

print(f"Model parameters: {sum(p.numel() for p in model.parameters())}")
print(f"Using optimized learning rate: {learning_rate:.0e}")
...
geom_str = "Irregular" if is_irregular else "Circular"
title_suffix = " (Grid Search Optimized with Learning Rate)" if GRID_SEARCH_ENABLED else ""
